...
# Likelihood calculation based on time delay and packet size
def calculate_likelihood(time_delay, packet_size):
    ...
    return L_delay_normal * L_size_normal, L_delay_malicious * L_size_malicious

# Bayes Theorem to determine if a packet is malicious
def is_malicious(time_delay, packet_size):
    L_normal, L_malicious = calculate_likelihood(time_delay, packet_size)
...
def packet_callback(packet):
    ...
    if IP in packet:
        ...
        if is_received:
            received_count += 1
            malicious = is_malicious(time_delay, packet_size)
            if malicious:
